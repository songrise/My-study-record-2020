...
def solver():
    ...
    # Generate a graph G from the set of states (nodes)
    G = genGraph(setAllStates)

    # The beginning state where all are on the east side.
    src = "EEEEEEE"
    # The terminating state when the problem is solved
    des = "WWWWWWW"

    # Generate the path by finding a shortest path from
    path = findShortestPath(G, src, des, path=[])
    # a source node src to a destination node.

    # Print the path in a reading-friendly format
    printPath(path)
...
def genGraph(S):
    ...
    for n in range(len(S)):
        if isAStateLegal(S[n]) == True:     # check whether a node (state) is legal
            # If legal, add n to the set of all legal states
...
def printPath(path):
    ...
    listPersons = ["", "green husband", "green wife",
                   "red husband", "red wife", "blue husband", "blue wife"]
    ...
    for nodeOnPath in range(len(path)-1):
        ...
        if len(whichPersonCrossing) == 1:
            printLine = printLine + \
                listPersons[int(whichPersonCrossing[0])] + " goes"
        else:
            printLine = printLine + \
                listPersons[int(whichPersonCrossing[0])] + " and " + \
                listPersons[int(whichPersonCrossing[1])] + " go"

        printLine = printLine + " from the " + \
...
def criticalNodes(G):
    """
    Return a  critical node that is  on the graph except the source node and destination
    node that has a maximum number of links to other nodes.

    Input: a graph G
    Output: a set of critical nodes and the number of links
    of each critical node
    """
    graph = G
    allNodes = [node for connection in graph.keys() for node in connection]
...
print("\nThe set of critical nodes is", setCritical, "and\
each has", connection, "connections.")